...
- Recovery covered: unclosed tags at EOF, mismatched close tags, inner unclosed tags closed by outer, stray `<` mid-tag, unterminated attribute values, unterminated comments/CDATA, broken sibling not poisoning later good siblings, plus all the markup-extension tolerance cases above. 218 tests in `XamlTolerantParserTests` (parser recovery + `ContextAt` + `EditAt` + `IdentifierSpanAt`).
