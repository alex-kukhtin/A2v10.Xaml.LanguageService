...
- **`AttributeValueContext` / `ExtensionArgValueContext` → the property's values** (`ValueEntries`). The owner property is resolved — the element for an attribute value, the extension type for a named-arg value — and the property named by the attribute/argument is looked up; if it is an enum its members are offered, if `Boolean` then `True`/`False`, otherwise nothing (string / type-converter values not yet wired). Entries are `XamlCompletionKind.EnumValue` with **no `Detail`** (no placeholder cliché). Nullable enums/bools work because `Nullable<T>` is unwrapped in `XamlProperty` (see above).

The no-op arms (`Outside` / `Text` / `Comment` / `CData` / `EndTagName`, and `TagInterior`) return empty. The scaffold convention: each arm hands its method the concentrated data it processes (element / attribute / extension), not the bare base context. `PropertyEntries(FullXamlType?)` is the shared property→entry builder; `ExtensionPropertyEntries` is the thin owner-from-extension wrapper over it; `ValueEntries(FullXamlType?, propertyName)` is the shared enum/bool value builder.
...
Done this round (formerly listed here): all value arms (`AttributeValueContext` / `ExtensionArgValueContext`) wired via `ValueEntries` with `Nullable<T>` unwrapping; **completion `TextEdit` / replacement ranges** — every item now carries an explicit range from `IdentifierSpanAt`, so the `:`-word-boundary problem for prefixed names (`my:Button`) and the quote-eating in value slots are both solved (see "Type / context provision").
...
- **Auto-popping the value list right after the auto-inserted closing quote (`Wrap="|"`) — DONE.** Typing `"` after `Wrap=` pops the enum-value list immediately (no Ctrl+Space), and committing keeps the closing quote. `"` lives legitimately in `CompletionHandler.TriggerCharacters` (alongside `<`).
  ...
  - **Fix (shipped):** when the attribute value is unterminated, append the matching closing quote to every completion item's `NewText`. `XamlContextProvider.Complete` sets `XamlCompletionResult.InsertSuffix = value.Quote` for `AttributeValueContext { Value.IsUnterminated: true }` (empty otherwise); `CompletionHandler` appends it (`NewText = e.Label + result.InsertSuffix`). Because the suffix is keyed off `IsUnterminated` — read from the **same** buffer the request carries — the count balances: a one-quote request adds one quote (→ `Background="White"` even if VS eats its own), a two-quote / Ctrl+Space request (`IsUnterminated == false`) adds nothing and never doubles. Proven working live in VS.
  - **Rejected en route:** `CompletionList.ItemDefaults.editRange` (OmniSharp 0.19.9 *does* expose it — `CompletionListItemDefaults.EditRange`) — tried live, did **not** override VS's applicable-span heuristic, so it doesn't address the race. `_vs_onAutoInsert` is VS-proprietary (we want VSCode too). LSP gives the server no way to open the popup itself, so the trigger char remains the only mechanism.
  - **Residual risk (not yet hit):** the nastiest corner of the race — request sees one quote, but by commit the closing quote *is* inserted *and* VS does *not* eat it — would double the quote. Not observed in practice; revisit only if it surfaces.
